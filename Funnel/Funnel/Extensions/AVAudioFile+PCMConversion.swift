...
extension AVAudioFile {
    ...
    func convertToPCM() throws -> Data {
        ...
        guard let outputFormat = AVAudioFormat(
            ...
            throw NSError(domain: "AudioConversion", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create output format"])
        }

        // Create converter
        guard let converter = AVAudioConverter(from: processingFormat, to: outputFormat) else {
            throw NSError(domain: "AudioConversion", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to create converter"])
        }
        ...
        guard let inputBuffer = AVAudioPCMBuffer(pcmFormat: processingFormat, frameCapacity: frameCount) else {
            throw NSError(domain: "AudioConversion", code: 3, userInfo: [NSLocalizedDescriptionKey: "Failed to create input buffer"])
        }
        ...
        guard let outputBuffer = AVAudioPCMBuffer(
            ...
            throw NSError(domain: "AudioConversion", code: 4, userInfo: [NSLocalizedDescriptionKey: "Failed to create output buffer"])
        }
        ...
        guard status != .error, error == nil else {
            throw error ?? NSError(domain: "AudioConversion", code: 5, userInfo: [NSLocalizedDescriptionKey: "Conversion failed"])
        }
