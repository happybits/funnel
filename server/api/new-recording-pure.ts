...
export async function newRecordingHandler(req: Request): Promise<Response> {
  try {
    ...
    console.log(
      `Processing audio file: ${audioFile.name}, size: ${audioFile.size}, type: ${audioFile.type}`,
    );
    ...
    if (providedTranscript && durationStr) {
      ...
      console.log(
        `Provided transcript length: ${transcript.length} characters, duration: ${duration}s`,
      );
    } else {
      // Transcribe audio using OpenAI Whisper
      console.log("Transcribing audio...");
      const transcriptionResponse = await openaiClient.transcribeAudio(
        audioFile,
      );
      ...
      console.log(
        `Transcription complete. Length: ${transcript.length} characters`,
      );
    }
